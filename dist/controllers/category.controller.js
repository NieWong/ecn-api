...
exports.updateCategory = [
    (0, auth_1.requireRole)("ADMIN"),
    (0, middleware_1.validateBody)(category_schema_1.updateCategorySchema),
    async (req, res, next) => {
        try {
            const category = await category_service_1.categoryService.update(String(req.params.id), req.body);
            res.status(200).json(category);
        }
        catch (error) {
            next(error);
